...
function buildHeaders({ token, isFormData, method = "GET", urlForCors } = {}) {
  ...
  const shouldSendDev =
    canDevHeaders &&
    !token &&
    (FORCE_DEV || DISABLE_AUTH || isLocalhostRuntime());
...
export const iamApi = {
  ...
  createUser: (payload, t) => {
    const obj = payload || {};
    const name = String(obj.nombreCompleto || "").trim();
    const email = String(obj.correoPersona || "")
      .trim()
      .toLowerCase();
    const roles = Array.isArray(obj.roles) ? obj.roles : [];
    const active = obj.active !== undefined ? !!obj.active : true;

    if (!name) return Promise.reject(new Error("Nombre requerido"));
