...
import { normalizePerm, validatePerm, pickUpdatable } from "../utils/permissions.helpers.js";
 ...
 */
export async function createPermissions(req, res, next) {
  try {
 ...
 */
export async function deletePermissionsById(req, res, next) {
  try {
    const { id } = req.params;
 ...
 * Sincroniza permisos desde el frontend de forma idempotente.
 * Body: { permissions: [{key,label,group,order?},...], dryRun?: boolean }
 */
export async function syncPermissions(req, res, next) {
  try {
